...
fn mesh_chunk_impl(
    ...
    // Preallocate vertex buffers based on the expected number of faces
    let mut positions = Vec::<[f32; 3]>::with_capacity(face_capacity * 4);
    let mut normals = Vec::<[f32; 3]>::with_capacity(face_capacity * 4);
    let mut uvs = Vec::<[f32; 2]>::with_capacity(face_capacity * 4);
    let mut indices = Vec::<u32>::with_capacity(face_capacity * 6);
...
pub(crate) fn mesh_chunk(
    ...
    mesh_chunk_impl(buffer, origin, step, tree, voxel_count * 6)
}
...
pub(crate) fn mesh_chunk_with_mask(
    ...
    let face_total: usize = face_counts.iter().map(|&c| c as usize).sum();
    mesh_chunk_impl(buffer, origin, step, tree, face_total)
}
