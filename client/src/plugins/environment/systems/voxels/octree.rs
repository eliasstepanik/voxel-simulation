...
impl SparseVoxelOctree {
    ...
    fn expand_root(&mut self, x: f32, y: f32, z: f32) {
        ...
        // Determine which direction to shift the octree center
        let sx = if x < old_center.x { -1.0 } else { 1.0 };
        let sy = if y < old_center.y { -1.0 } else { 1.0 };
        let sz = if z < old_center.z { -1.0 } else { 1.0 };

        self.center += Vec3::new(sx, sy, sz) * (old_size * 0.5);
        self.size *= 2.0;
        self.max_depth += 1;
    ...
    pub fn has_neighbor(
        ...
        let neighbor_world = self.denormalize_voxel_center(neighbor);
